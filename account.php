                ...
                <table border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px;">
                    ...
                    <tr>
                        <td bgcolor="#ffffff" align="left" style="padding: 0px 30px 40px 30px; color: #666666; font-family: 'Lato', Helvetica, Arial, sans-serif; font-size: 18px; font-weight: 400; line-height: 25px;">
                            ...
                            <input type="password" class="form-control" id="confirm_password_input" required>
                            <div class="invalid-feedback">
                                Please enter your password
                            </div>
                        </td>
                                ...
                                <tr>
                                    <td bgcolor="#ffffff" align="center" style="padding: 20px 30px 60px 30px;">
                                        <table border="0" cellspacing="0" cellpadding="0">
                                            <tr>
                                                <td align="center" style="border-radius: 3px;"><button id="btn-reset" class="btn btn-warning" style="font-size: 20px; font-family: Helvetica, Arial, sans-serif; color: #ffffff; text-decoration: none; color: #ffffff; text-decoration: none; padding: 15px 25px; border-radius: 2px;  display: inline-block;">Confirm Account</button></td>
                                            </tr>
                                        </table>
                                    </td>
                                </tr>
        ...
        document.getElementById('btn-reset').addEventListener('click', function() {
            ...
            if (!password.match(passwordRegex)) {
                Swal.fire({
                    title: 'Thông báo',
                    icon: 'error',
                    text: 'The password must be at least 6 characters long and include at least one digit, one lowercase letter, and one uppercase letter.',
                ...
                Swal.fire({
                    title: 'Oops..',
                    icon: 'error',
                    text: 'Password or confirm password cannot empty',
                    confirmButtonText: 'Đồng ý',
                    reverseButtons: true
                }).then((result) => {
                    if (result.isConfirmed) {
                ...
                if (password === confirmPassword) {
                    const token = <?php $token = $_GET['reset'] ?? null;
                    ...
                    xmlhttp.onreadystatechange = function() {
                        ...
                        if (this.readyState == 4 && this.status == 200) {
                            ...
                            } else if (json.status === '404' || json.status === '500') {
                                Swal.fire({
                                    title: 'Oops..',
                                    icon: 'error',
                    ...
                    xmlhttp.open("POST", "http://127.0.0.1:3456/lnstagram/api/verify.php", true);
                    xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
                    xmlhttp.send("token=" + token + "&password=" + password);
                } else {
                    Swal.hideLoading();
                    Swal.fire({
                        title: 'Oops..',
